...
// The analytics hub's body (tiles, charts, history) — split out so Home can embed it
// directly under the day-to-day cards, not just link to it.
export function StatsSections({ showBodyWeight = true } = {}) {
  ...
  return <>
    ...
    {(() => {
      ...
      // Home already shows its own Body weight card, so it passes showBodyWeight={false}
      // to avoid repeating it here — the standalone Stats page keeps both, side by side.
      return showBodyWeight ? (
        <div className="cols">
          <div className="card">
            <div className="row between" style={{ marginBottom: 8 }}>
...
// Stats = the standalone page at /stats (same content as StatsSections, with its own header).
