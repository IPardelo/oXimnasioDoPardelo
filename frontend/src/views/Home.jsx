...
import { StatsSections } from './Stats.jsx'
...
export default function Home() {
  ...
  return <div className="narrow">
    <div className="hdr">
      <div><h1>{user ? t('Hi {0}', user.name) : 'O Ximnasio do Pardelo'}</h1><div className="sub">{today.toLocaleDateString(dateLocale(), { weekday: 'long', day: 'numeric', month: 'long' })}</div></div>
    ...
    {/* Stats moved out of the tab bar (issue: 6 tabs felt cramped) — its full content now
        lives right here, after the day-to-day cards above, instead of behind a link.
        Body weight is already shown in its own card above, so it's hidden here to avoid
        repeating it. */}
    <StatsSections showBodyWeight={false} />
  </div>
}
