...
export default function Settings() {
  ...
  const doExport = async () => {
    ...
    const name = 'ximnasioPardelo-backup-' + todayISO() + '.json'
  ...
  const doImport = ev => {
    ...
    rd.onload = () => {
      try {
        ...
        if (!data.workouts || !data.routines) throw new Error('not an O Ximnasio do Pardelo backup')
  ...
  const signOutEverywhere = () => confirmSheet({
    ...
    message: t('Signs this profile out on every device, including this one. You can sign in again anytime with Google or your username and password.'),
  ...
  return <div className="narrow">
    ...
    {/* ---------- account (demo, static and mobile builds have nothing to sign in to) ---------- */}
    <Section title={MOBILE ? t('Your data') : DEMO ? t('Demo') : NO_BACKEND ? t('Your data') : t('Account')}>
      {MOBILE ? <>
        <Row icon="lock" iconTint="var(--acc)" title={t('All data stays on this phone')} subtitle={t('No account, no cloud — back it up anytime with Export below.')} />
        <Row icon="rocket" iconTint="var(--indigo)" title={t('Self-host O Ximnasio do Pardelo')} subtitle={t('Sign in with Google or a username, sync across your devices, your own data.')} accessory="chevron"
        ...
        <Row icon="lock" iconTint="var(--acc)" title={t('All data stays in this browser')} subtitle={t('No account, no cloud — back it up anytime with Export below.')} />
        <Row icon="rocket" iconTint="var(--indigo)" title={t('Self-host O Ximnasio do Pardelo')} subtitle={t('Sign in with Google or a username, sync across your devices, your own data.')} accessory="chevron"
          onClick={() => window.open(REPO, '_blank', 'noopener')} />
      </> : user && <>
        <Row icon="personCircle" iconTint="var(--grey)" title={user.name} subtitle={t('Data syncs to this profile.')} />
    ...
    {/* ---------- appearance ---------- */}
    <Section title={t('Appearance')} footer={DEMO || MOBILE || NO_BACKEND ? undefined : t('synced with your profile')}>
    ...
    {!MOBILE && <Section title={t('Tip')}>
      <Row icon="lightbulb" iconTint="var(--yellow)"
        ...
        subtitle={t('to install O Ximnasio do Pardelo as a full-screen app.') + ' ' + (user ? t('Your data syncs with your profile — sign in anywhere to see it.') : t('Guest data stays on this device — export a backup now and then!'))} />
    </Section>}

    <div className="dim small" style={{ textAlign: 'center', marginTop: 4, lineHeight: 1.6 }}>
      Desarrollado por <a href="https://ipardelo.es" target="_blank" rel="noopener">ipardelo</a><br />
      <a href="https://github.com/IPardelo/oXimnasioDoPardelo" target="_blank" rel="noopener">source code</a> · exercise data: hasaneyldrm/exercises-dataset (CC)
    </div>
  </div>
}
  ...
  return <>
    <Section
      ...
      footer={on && S.reminder?.on
        ...
        : null}
    >
      <Row icon="bell" iconTint="var(--red)" title={t('Push notifications')} subtitle={t('Rest-timer alerts, even if O Ximnasio do Pardelo is closed.')}>
