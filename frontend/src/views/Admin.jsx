...
// Admin-only operator dashboard (owner admin flag; guarded again server-side).
...
function UserDetail({ id, onChanged, close }) {
  ...
  useEffect(() => { api('/api/admin/user.php?id=' + encodeURIComponent(id)).then(setD).catch(e => toast(e.message)) }, [id])
  if (!d) return <div className="muted small">Loading…</div>
  const u = d.user
  const setDisabled = disabled => {
    api('/api/admin/user-disable.php', { method: 'POST', body: JSON.stringify({ id: u.id, disabled }) })
...
// A single shared code required to create an account (ximnasioPardelo_configuracion.invite_code) —
// not a list of one-time codes. Rotating it only blocks NEW sign-ups with the old code; accounts
// already created keep working, since it's never re-checked after registration.
function InviteCodeCard({ code, reload }) {
  const toast = useUI(s => s.toast)
  const copy = () => { navigator.clipboard?.writeText(code).catch(() => {}); toast('Copied ' + code) }
  const regen = () => confirmSheet({
    title: 'Generate a new invite code?',
    message: 'The current code stops working for new sign-ups right away. Accounts already created are not affected.',
    confirmText: 'Generate new code',
    onConfirm: () => api('/api/admin/invite-code.php', { method: 'POST', body: '{}' })
      .then(({ code }) => { navigator.clipboard?.writeText(code).catch(() => {}); toast('New code ' + code + ' created & copied'); reload() })
      .catch(e => toast(e.message)),
  })
  return <div className="card">
    <div className="row between"><h2 style={{ margin: 0 }}>Invite code</h2>
      <Button variant="primary" size="sm" onClick={regen} icon="reset">Generate new</Button></div>
    <div className="small muted" style={{ margin: '6px 0 10px' }}>Required to create an account — share it with whoever you want to invite.</div>
    {code
      ? <div className="row between" style={{ padding: '7px 2px' }}>
          <span style={{ fontFamily: 'ui-monospace,SFMono-Regular,Menlo,monospace', fontWeight: 600, letterSpacing: '.1em', fontSize: '1.05rem' }}
            onClick={copy}>{code}</span>
          <button className="iconbtn" onClick={copy} aria-label="copy"><Icon name="clipboard" /></button>
        </div>
...
export default function Admin() {
  ...
  const loadUsers = () => api('/api/admin/users.php').then(d => setUsers(d.users)).catch(e => toast(e.message || 'Failed to load'))
  const loadInviteCode = () => api('/api/admin/invite-code.php').then(d => setInviteCode(d.code)).catch(() => {})
  // poll every 15s so the "training now" section stays live without a manual refresh
  useEffect(() => { if (!user?.admin) return; loadUsers(); loadInviteCode(); const iv = setInterval(loadUsers, 15000); return () => clearInterval(iv) }, [])
  ...
  return <div className="narrow">
    <div className="hdr">
      ...
      <button className="iconbtn" onClick={() => { loadUsers(); loadInviteCode() }} aria-label="refresh">↻</button>
    </div>
    ...
    <InviteCodeCard code={inviteCode} reload={loadInviteCode} />
