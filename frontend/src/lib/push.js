...
export async function enablePush() {
  ...
  const { key } = await api('/api/push-public-key.php')
  const subscription = await reg.pushManager.subscribe({ userVisibleOnly: true, applicationServerKey: urlBase64ToUint8Array(key) })
  await api('/api/push-subscribe.php', { method: 'POST', body: JSON.stringify({ subscription: subscription.toJSON() }) })
}
...
export async function disablePush() {
  ...
  await api('/api/push-unsubscribe.php', { method: 'POST', body: JSON.stringify({ endpoint: sub.endpoint }) }).catch(() => {})
}

export const sendTestPush = () => api('/api/push-test.php', { method: 'POST', body: '{}' })
