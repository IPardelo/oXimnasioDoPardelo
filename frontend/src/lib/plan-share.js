...
//
// Two jobs:
//  1. A small, self-contained file a friend can import into THEIR O Ximnasio do Pardelo — just the
...
export function buildPlanBundle(S, name) {
  ...
  return { ximnasioPardelo_plan: PLAN_FMT, exported: todayISO(), name: name || '', week, routines, customEx }
}
...
export function parsePlan(raw) {
  ...
  if (!data || !data.ximnasioPardelo_plan || !Array.isArray(data.routines)) {
    throw new Error(t('this isn’t a O Ximnasio do Pardelo plan file'))
  }
...
<body><div class="doc">
  <header>
    <div class="kicker">O Ximnasio do Pardelo</div>
  ...
  ${body}
  <footer>${esc(t('Made with O Ximnasio do Pardelo'))} · ipardelo.es</footer>
</div></body></html>`
}
