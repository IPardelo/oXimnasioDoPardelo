...
const pushRestTimer = sec => { if (useStore.getState().user) api('/api/push-rest-timer.php', { method: 'POST', body: JSON.stringify({ seconds: sec }) }).catch(() => {}) }
const cancelPushRestTimer = () => { if (useStore.getState().user) api('/api/push-rest-timer-cancel.php', { method: 'POST', body: '{}' }).catch(() => {}) }
