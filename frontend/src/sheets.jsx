...
function CustomExForm({ existing, prefill, onDone, close }) {
  ...
  return <>
    ...
    <div className="chips chips-fill" style={{ margin: '12px 0' }}>
...
function ExercisePicker({ onPick, close }) {
  ...
  return <>
    ...
    <div className="chips chips-fill" style={{ margin: eqOpts.length > 1 ? '10px 0 6px' : '10px 0' }}>
    ...
    {eqOpts.length > 1 && <hr className="divider" />}
    {eqOpts.length > 1 && <div className="chips chips-fill" style={{ marginBottom: 10 }}>
...
function PlanTools({ close }) {
  ...
  const exportFile = async () => {
    ...
    const name = 'ximnasioPardelo-plan-' + todayISO() + '.json'
  ...
  return <>
    ...
    <div className="dim small" style={{ margin: '7px 2px 0', lineHeight: 1.4 }}>{t('A small file a friend imports into their own O Ximnasio do Pardelo — routines only, none of your workouts or weigh-ins.')}</div>
    {!MOBILE && <>
      <div style={{ height: 12 }} />
